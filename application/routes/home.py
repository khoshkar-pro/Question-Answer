...
def home():
    user = user_session()
    db = db_get()
    home_curr = db.execute('select questions.id, questions.question_text as question_text, questions.answer_text as answer_text, askers.name as asker, experts.name as expert from questions join users as askers on askers.id = questions.asked_by_id join users as experts on experts.id = questions.expert_id where questions.answer_text is not null')
